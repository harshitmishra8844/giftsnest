...
const buildCustomerOrderEmailContent = (order, { storeName = "Niyora Gifts" } = {}) => {
  const orderCode = order.orderCode || `ORD-${order._id.toString().slice(-8).toUpperCase()}`;
  const orderDate = order.createdAt
    ? new Date(order.createdAt).toLocaleString("en-IN", {
        timeZone: "Asia/Kolkata",
  ...
  const shopUrlStr = shopUrl();
  const trackOrderUrl = shopUrlStr ? `${shopUrlStr}/track-order` : "";
  const trackBtn = trackOrderUrl
    ? `<a href="${encodeURI(trackOrderUrl)}" style="display: inline-block; background-color: #059669; color: #ffffff; padding: 14px 32px; font-size: 15px; font-weight: 700; text-decoration: none; border-radius: 9999px; box-shadow: 0 4px 6px -1px rgba(5, 150, 105, 0.2); text-align: center; margin-top: 10px;">Track Your Order</a>`
    : "";

  const isCod = order.paymentMethod === "COD";

  const text = `Order Confirmed - Code: ${orderCode}
...
Subtotal: ₹${subtotal}
Discount: ₹${discount} ${order.couponCode ? `(Code: ${order.couponCode})` : ""}
Total ${isCod ? "Payable (Cash on Delivery)" : "Paid"}: ₹${total}

You can track your order here: ${trackOrderUrl}
— ${storeName}
`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Order Confirmation - ${escapeHtml(orderCode)}</title>
</head>
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f3f4f6;padding:30px 10px;">
    <tr>
      <td align="center">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#ffffff;border-radius:16px;box-shadow:0 4px 10px rgba(0,0,0,0.05);overflow:hidden;max-width:600px;text-align:left;">
          ...
          <tr>
            <td align="center" style="padding: 24px 20px; border-bottom: 1px solid #f3f4f6;">
              <span style="font-size: 24px; font-weight: 800; color: #064e3b; letter-spacing: -0.03em;">Niyora <span style="color: #059669;">Gifts</span></span>
              <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.15em; color: #4b5563; margin-top: 4px; font-weight: 600;">Order Confirmation</div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 24px 30px 12px 30px;">
              <h2 style="margin: 0; font-size: 20px; font-weight: 700; color: #111827;">Thank you for your order!</h2>
              <p style="margin: 6px 0 0 0; font-size: 14px; color: #4b5563; line-height: 1.5;">Hi ${escapeHtml(customerName)}, we've received your order and are getting it ready. We'll notify you as soon as your items ship.</p>
            </td>
          </tr>
            ...
            <td style="padding: 10px 30px;">
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f9fafb; border-radius: 12px; border: 1px solid #e5e7eb; padding: 16px 20px;">
                ...
                <tr>
                  ...
                  <td style="font-size: 13px; color: #111827; font-weight: 700; padding-bottom: 8px; text-align: right; text-transform: uppercase;">${escapeHtml(order.paymentMethod || "Online")}</td>
                </tr>
                <tr>
                  <td style="font-size: 13px; color: #4b5563; padding-bottom: 8px;">Payment Status:</td>
                  <td style="font-size: 13px; color: #047857; font-weight: 700; padding-bottom: 8px; text-align: right; text-transform: uppercase;">${escapeHtml(order.paymentStatus || "Paid")}</td>
          ...
          <tr>
            <td style="padding: 12px 30px;">
              <h3 style="margin: 0 0 10px 0; font-size: 15px; font-weight: 700; color: #111827; border-bottom: 2px solid #f3f4f6; pb: 4px;">Delivery Address</h3>
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size: 13.5px; color: #374151; line-height: 1.6;">
                <tr>
                  <td width="30%" style="font-weight: 600; color: #4b5563; padding: 4px 0;">Name:</td>
                  <td width="70%" style="padding: 4px 0;">${escapeHtml(customerName)}</td>
                ...
                <tr>
                  <td style="font-weight: 600; color: #4b5563; padding: 4px 0; vertical-align: top;">Address:</td>
                  <td style="padding: 4px 0;">${escapeHtml(fullAddress)}</td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding: 12px 30px;">
              <h3 style="margin: 0 0 10px 0; font-size: 15px; font-weight: 700; color: #111827; border-bottom: 2px solid #f3f4f6; pb: 4px;">Items Ordered</h3>
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-collapse: collapse; text-align: left;">
                <thead>
                  <tr style="border-bottom: 2px solid #e5e7eb; background-color: #f9fafb;">
                    <th style="padding: 8px; font-size: 12px; font-weight: 700; color: #4b5563; text-transform: uppercase;">Product</th>
          ...
          <tr>
            <td style="padding: 10px 30px 20px 30px;">
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-top: 2px solid #e5e7eb; padding-top: 12px;">
                ...
                ` : ''}
                <tr style="border-top: 1px solid #e5e7eb;">
                  <td style="font-size: 16px; color: #111827; font-weight: 700; padding: 12px 0 4px 0; text-align: right;">Total ${isCod ? "Payable" : "Paid"}:</td>
                  <td style="font-size: 18px; color: #059669; font-weight: 800; padding: 12px 0 4px 0; text-align: right;">₹${total}</td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td align="center" style="padding: 10px 30px 30px 30px; border-top: 1px solid #f3f4f6;">
              ${trackBtn}
              <p style="margin: 10px 0 0 0; font-size: 12px; color: #6b7280;">You can check your order status anytime using the link above.</p>
            </td>
          </tr>
          ` : ''}
  ...
  return {
    subject: `Order Confirmed: ${orderCode} - ${storeName}`,
    text,
    html,
  };
};
...
const sendOrderConfirmationToCustomer = async (order) => {
  ...
  // Populate user details if missing or not an object
  let orderData = order;
  let customerEmail = "";

  if (order.userId && typeof order.userId === "object" && order.userId.email) {
    customerEmail = String(order.userId.email).trim().toLowerCase();
    ...
    try {
      ...
      console.error("[email] Error populating order user for customer confirmation:", dbErr);
    }
  }

  if (!customerEmail) {
    console.warn("[email] Cannot send customer order confirmation: Customer email is missing.");
    return null;
  }
  ...
  const { subject, html, text } = buildCustomerOrderEmailContent(orderData, {
    storeName,
  });
  ...
  console.info(`[email] Dispatching customer order confirmation to: ${customerEmail}`);
...
const sendCancellationRequestNotificationToAdmin = async (order) => {
  const config = getSmtpConfig();
  if (!config.provider) {
    const err = new Error("EMAIL_NOT_CONFIGURED");
    err.code = "EMAIL_NOT_CONFIGURED";
  ...
  const orderCode = orderData.orderCode || `ORD-${orderData._id.toString().slice(-8).toUpperCase()}`;
  const customerName = orderData.address?.fullName || "Valued Customer";
  const reason = orderData.cancellationRequest?.reason || "Not specified";
  const details = orderData.cancellationRequest?.details || "Not specified";

  const subject = `[CANCELLATION REQUEST] ${orderCode} - ${customerName}`;
  const text = `Cancellation Request for Order ${orderCode}
  
Customer ${customerName} has requested a cancellation for their order.
...
<head>
  <meta charset="UTF-8">
  <title>Cancellation Request - ${escapeHtml(orderCode)}</title>
</head>
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f3f4f6;padding:30px 10px;">
    <tr>
      <td align="center">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#ffffff;border-radius:16px;box-shadow:0 4px 10px rgba(0,0,0,0.05);overflow:hidden;max-width:600px;text-align:left;">
          <tr><td height="6" style="background-color:#dc2626;"></td></tr>
          <tr>
            <td align="center" style="padding: 24px 20px; border-bottom: 1px solid #f3f4f6;">
              <span style="font-size: 24px; font-weight: 800; color: #064e3b; letter-spacing: -0.03em;">Niyora <span style="color: #059669;">Gifts</span></span>
              <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.15em; color: #dc2626; margin-top: 4px; font-weight: 600;">Cancellation Request</div>
            </td>
          </tr>
          <tr>
            <td style="padding: 24px 30px 12px 30px;">
              <h2 style="margin: 0; font-size: 18px; font-weight: 700; color: #111827;">Cancellation Requested!</h2>
              <p style="margin: 6px 0 0 0; font-size: 14px; color: #4b5563; line-height: 1.5;">An order cancellation request has been submitted by the customer. Details are below:</p>
            </td>
          </tr>
          <tr>
            <td style="padding: 10px 30px;">
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f9fafb; border-radius: 12px; border: 1px solid #e5e7eb; padding: 16px 20px; font-size: 13.5px; line-height: 1.6; color: #374151;">
                <tr>
                  <td width="30%" style="font-weight: 600; color: #4b5563;">Order Code:</td>
                  <td width="70%" style="font-weight: 700; color: #111827;">${escapeHtml(orderCode)}</td>
                </tr>
                <tr>
                  <td style="font-weight: 600; color: #4b5563;">Customer:</td>
                  <td>${escapeHtml(customerName)}</td>
                </tr>
                <tr>
                  <td style="font-weight: 600; color: #4b5563; vertical-align: top;">Reason:</td>
                  <td style="color: #b91c1c; font-weight: 600;">${escapeHtml(reason)}</td>
                </tr>
                <tr>
                  <td style="font-weight: 600; color: #4b5563; vertical-align: top;">Details:</td>
          ...
          <tr>
            <td align="center" style="padding: 20px 30px 30px 30px;">
              <a href="${encodeURI(adminDashboardUrl)}" style="display: inline-block; background-color: #dc2626; color: #ffffff; padding: 12px 28px; font-size: 14px; font-weight: 700; text-decoration: none; border-radius: 9999px;">Review Request</a>
            </td>
          </tr>
        </table>
      </td>
...
const sendCancellationStatusNotificationToCustomer = async (order, status, adminNote = "") => {
  const config = getSmtpConfig();
  if (!config.provider) {
    const err = new Error("EMAIL_NOT_CONFIGURED");
    err.code = "EMAIL_NOT_CONFIGURED";
  ...
  if (!customerEmail) {
    console.warn("[email] Cannot send customer cancellation notification: Email missing.");
    return null;
  }
  ...
  const orderCode = orderData.orderCode || `ORD-${orderData._id.toString().slice(-8).toUpperCase()}`;
  const customerName = orderData.address?.fullName || "Valued Customer";
  const isApproved = status === "Approved" || order.status === "Cancelled";

  const subject = isApproved 
    ? `Order Cancelled: ${orderCode} - ${storeName}` 
    : `Cancellation Request Update: ${orderCode} - ${storeName}`;

  const text = isApproved
    ? `Your order ${orderCode} has been successfully cancelled.
    
Hi ${customerName}, we have successfully processed your order cancellation request.
${order.paymentStatus === "Paid" ? "Since you made an online payment, a refund has been initiated and will reflect in your account within 5-7 business days." : "No payment was charged."}
Admin note: ${adminNote || "Approved"}
`
    : `Update on your cancellation request for order ${orderCode}.
    
Hi ${customerName}, your cancellation request has been reviewed. Unfortunately, we are unable to cancel your order at this time.
Admin note: ${adminNote || "Unable to cancel order at this stage."}
`;
...
<head>
  <meta charset="UTF-8">
  <title>Cancellation Update - ${escapeHtml(orderCode)}</title>
</head>
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f3f4f6;padding:30px 10px;">
    <tr>
      <td align="center">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#ffffff;border-radius:16px;box-shadow:0 4px 10px rgba(0,0,0,0.05);overflow:hidden;max-width:600px;text-align:left;">
          <tr><td height="6" style="background-color:${isApproved ? '#b91c1c' : '#d97706'};"></td></tr>
          <tr>
            <td align="center" style="padding: 24px 20px; border-bottom: 1px solid #f3f4f6;">
              <span style="font-size: 24px; font-weight: 800; color: #064e3b; letter-spacing: -0.03em;">Niyora <span style="color: #059669;">Gifts</span></span>
              <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.15em; color: #4b5563; margin-top: 4px; font-weight: 600;">Cancellation Review</div>
            </td>
          </tr>
          <tr>
            <td style="padding: 24px 30px 12px 30px;">
              <h2 style="margin: 0; font-size: 18px; font-weight: 700; color: #111827;">
                ${isApproved ? "Order Cancelled Successfully" : "Cancellation Request Update"}
              </h2>
              <p style="margin: 6px 0 0 0; font-size: 14px; color: #4b5563; line-height: 1.5;">
                Hi ${escapeHtml(customerName)}, here is the status of your cancellation request for order <strong>${escapeHtml(orderCode)}</strong>:
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding: 10px 30px;">
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f9fafb; border-radius: 12px; border: 1px solid #e5e7eb; padding: 16px 20px; font-size: 13.5px; line-height: 1.6; color: #374151;">
                <tr>
                  <td width="35%" style="font-weight: 600; color: #4b5563;">Status:</td>
                  <td width="65%" style="font-weight: 700; color: ${isApproved ? '#b91c1c' : '#d97706'}; text-transform: uppercase;">
                    ${isApproved ? "Cancelled" : "Declined"}
                  </td>
                </tr>
                ${order.paymentStatus === "Paid" && isApproved ? `
                <tr>
                  <td style="font-weight: 600; color: #4b5563;">Refund Status:</td>
                  <td style="color:#059669; font-weight: 600;">Initiated (5-7 business days)</td>
                </tr>
                ` : ''}
                <tr>
                  <td style="font-weight: 600; color: #4b5563; vertical-align: top;">Note from Store:</td>
                  <td>${escapeHtml(adminNote || (isApproved ? "Approved" : "Your order could not be cancelled."))}</td>
                </tr>
              </table>
            </td>
          </tr>
          <tr>
            <td align="center" style="padding: 20px 30px 30px 30px; border-top: 1px solid #f3f4f6;">
              <p style="margin: 0; font-size: 12px; color: #6b7280;">If you have any questions, please reply to this email or contact support.</p>
            </td>
          </tr>
        </table>
      </td>
...
module.exports = {
  ...
  sendCancellationRequestNotificationToAdmin,
  sendCancellationStatusNotificationToCustomer,
};
